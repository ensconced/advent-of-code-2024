...
bool part2_make_interval_safe(size_t j, parsed_line line, bool *increasing,
  ...
  if (*dampeners > 0) {
    // if we're looking at the first or last interval, it's always fixable
    if (j == 1 || j == line.length - 1) {
      (*dampeners)--;
      return true;
    }

    // what if we removed prev?
...
bool part2_line_is_safe(parsed_line line, int *dampeners) {
  if (line.length < 2)
    return true;
...
int part2(char *input_path) {
  ...
  for (size_t i = 0; i < input.length; i++) {
    int dampeners = 1;
    parsed_line line = input.lines[i];
    if (part2_line_is_safe(line, &dampeners)) {
      safe_lines++;
    }
  }
